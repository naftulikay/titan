...
resource aws_route hub_ipv4 {
  ...
  count = var.peering_include_public ? var.subnets_per_layer * 6 : var.subnets_per_layer * 5

  destination_cidr_block = var.hub_cidr_block

  route_table_id = var.peering_include_public ? module.network.all_subnet_route_table_ids[count.index] : module.network.private_route_table_ids[count.index]
