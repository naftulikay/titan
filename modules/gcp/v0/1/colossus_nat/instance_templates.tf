...
resource "google_compute_instance_template" "primary" {
  ...
  network_interface {
    ...
    address = "${element(google_compute_address.private_primary.*.address, count.index)}"
    access_config {
...
resource "google_compute_instance_template" "secondary" {
  ...
  network_interface {
    subnetwork = "${var.dmz_subnet_id}"
    address = "${element(google_compute_address.private_secondary.*.address, count.index)}"
    access_config {
