...
# remove default ip forwarding rules
grep -RiPl '^net\.ipv4\.ip_forward.*' /etc/sysctl.conf /etc/sysctl.d | while read file ; do
  sed -i '/^net\.ipv4\.ip_forward.*/d' "$${file}"
done

# create nat forwarding rules
cat <<EOF > /etc/sysctl.d/50-nat.conf
# allow ip forwarding for NAT
net.ipv4.ip_forward = 1
EOF

# load sysctl settings from disk
sysctl --system

# inject NAT route into iptables
...
# additional startup scripts specified by user
${additional_startup_script}
